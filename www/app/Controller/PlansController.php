<?php
	class BodybuildingplansController extends AppController {
		...
		public function msf_step($stepNumber) {
			...
			$outcomedata = $this->Bodybuildingplan->Outcome->find('list', array('fields' => array('id', 'outcomename')));
			$this->set('outcome', $outcomedata);

			$dietdata = $this->Bodybuildingplan->Dietplan->find('list', array('fields' => array('id', 'dietname')));
			$this->set('diet', $dietdata);

			$workoutdata = $this->Bodybuildingplan->find('list', array('fields' => array('id', 'bodybuildingname')));
			...
			if (!file_exists(APP.'View'.DS.'Users'.DS.'msf_step_'.$stepNumber.'.ctp')) {
				$this->redirect('/bodybuildingplans/index');
			}
			...
			if ($stepNumber > $maxAllowed) {
				$this->redirect('/bodybuildingplans/msf_step/'.$maxAllowed);
			} else {
			...
			if ($this->request->is('post')) {
				 ...
				 */
				$this->Bodybuildingplan->User->set($this->request->data);
				 ...
				 */
				if ($this->Bodybuildingplan->User->validates()) {
					...
					if ($stepNumber < $this->Session->read('form.params.steps')) {
						 ...
						 */
						$this->Bodybuildingplan->User->save($currentSessionData);
